...
export type TakeArray<A extends unknown[], N extends number> =
	A extends { length: N } ? A : A extends [ ...infer P, unknown ] ? TakeArray<P, N> : never;
...
export type SkipArray<A extends unknown[], N extends number> =
	N extends 0 ? A : A extends [ unknown, ...infer S ] ? SkipArray<S, MinusOne<N>> : never;
 ...
 */
export type Slice<A extends T[], B extends number, E extends number, T = unknown> =
 ...
 */
export type FillLeft<T extends V[], L extends number, I extends V, V = unknown, R extends V[] = T> = 
	GreatThenOrEquals<ArrayLength<R>, L> extends true ? R : FillLeft<T, L, I, V, [I, ...R]>;

/**
 * 向数组右侧添加指定类型，获取一个指定长度的新数组
 */
export type FillRight<T extends V[], L extends number, I extends V, V = unknown, R extends V[] = T> = 
